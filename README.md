...
![Abstract illustration of AI audio generation systems](assets/readme-hero.png)
...
- **[CharaVox](https://charavox.com)** – Multilingual character voice generation and voice cloning platform powered by the open-source VoxCPM model. Role-specific voice pages for support agents, podcast hosts, sales reps, and game characters in 6 languages (English, Chinese, Japanese, Korean, Spanish, Portuguese).
...
- **[Fish Audio](https://fish.audio)** – Hosted TTS and voice cloning API backed by the open-source Fish Speech model. S2 Pro supports 80+ languages, voice cloning from 15s of audio, WebSocket streaming. Python and JavaScript SDKs. [Docs](https://docs.fish.audio) | SDK: [Python](https://github.com/fishaudio/fish-audio-python), JavaScript
- **[Gemini TTS (Google)](https://ai.google.dev/gemini-api/docs/speech-generation)** – Gemini 2.5 Flash/Pro TTS and Gemini 3.1 Flash TTS via the Gemini API. 30 voices, 100+ languages, natural language style control, audio tags ([whispers], [laughs]), multi-speaker (2 voices). Separate from Google Cloud TTS. [Docs](https://ai.google.dev/gemini-api/docs/speech-generation) | SDK: Python (google-genai), JavaScript (@google/genai)
...
- **[MiniMax Audio](https://platform.minimax.io)** – Commercial TTS and voice cloning API. Speech-2.8-HD model supports 40+ languages and 7 emotion modes. Voice cloning from 10s reference clip via REST API. [Docs](https://platform.minimax.io/docs/guides/speech-voice-clone)
...
- **[xAI Grok TTS](https://x.ai/api/voice)** – Commercial TTS API launched April 2026. 5 expressive voices, 20 languages, inline speech tags (laughter, whispers), WebSocket streaming, and voice cloning via custom-voices endpoint. $4.20/1M chars. [Docs](https://docs.x.ai/developers/model-capabilities/audio/voice) | SDK: [Python](https://github.com/xai-org/xai-sdk-python)
...
- **[ChatTTS](https://github.com/2noise/ChatTTS)** – Conversational TTS with multi-speaker synthesis, prosody control, and emotion tags (laughter, pauses). pip install. 39k stars. Code AGPL-3.0; model weights CC-BY-NC-4.0 (non-commercial). [Docs](https://huggingface.co/2Noise/ChatTTS) | SDK: Python (pip install ChatTTS)
...
- **[MOSS-TTS (OpenMOSS)](https://github.com/OpenMOSS/MOSS-TTS)** – Open-source TTS model family (Apache-2.0). Five variants covering voice cloning, dialogue, voice design, realtime streaming, and sound effects. 20 languages; MOSS-TTS-Nano runs on 4 CPU cores. Launched Feb 2026. [Docs](https://huggingface.co/collections/OpenMOSS/moss-tts-family) | SDK: Python (pip install -e '.[torch-runtime]')
- **[Orpheus-TTS](https://github.com/canopyai/Orpheus-TTS)** – Llama-3B-based TTS with guided emotion tags (laughter, sighs, etc.), zero-shot voice cloning, and 200ms streaming latency. Trained on 100k hours of English speech. [Docs](https://huggingface.co/canopylabs/orpheus-3b-0.1-ft) | SDK: Python (pip install orpheus-speech)
- **[OuteTTS](https://github.com/edwko/OuteTTS)** – Extends any LLM with TTS via a pure-language-model approach. 0.6B and 1B variants. One-shot voice cloning, multilingual, no phoneme pre-processing required. Backends for CUDA, ROCm, Metal, Vulkan. [Docs](https://huggingface.co/OuteAI/OuteTTS-1.0-1B) | SDK: Python (pip install outetts)
...
- **[TADA (Hume AI)](https://github.com/HumeAI/tada)** – Open-source speech-language model (1B/3B) with 1:1 text-acoustic alignment. Zero content hallucinations, RTF 0.09 (5x faster than comparable LLM-TTS), 10 languages. Weights under Llama 3.2 license; code MIT. [Docs](https://huggingface.co/HumeAI/tada-3b-ml) | SDK: Python (pip install hume-tada)
- **[VibeVoice (Microsoft)](https://github.com/microsoft/VibeVoice)** – MIT open-source TTS family (1.5B) for long-form multi-speaker conversational audio up to 90 min. Next-token diffusion at 7.5 Hz. ICLR 2026 Oral. Also ships a Realtime-0.5B streaming variant. [Docs](https://microsoft.github.io/VibeVoice/)
...
- **[AudioX](https://github.com/ZeyueT/AudioX)** – Unified diffusion transformer for anything-to-audio generation. Takes text, video, image, music, or audio as input; outputs general audio or music. Accepted at ICLR 2026. CC-BY-NC. [Docs](https://huggingface.co/HKUSTAudio/AudioX)
- **[DiffRhythm](https://github.com/ASLP-lab/DiffRhythm)** – Latent diffusion model for end-to-end full-song generation (vocals + accompaniment). Generates up to 4m45s of music in ~10s. Trained on 1M songs (60k hours). Apache-2.0. [Docs](https://huggingface.co/ASLP-lab/DiffRhythm)
- **[DiffSinger (OpenVPI)](https://github.com/openvpi/DiffSinger)** – Advanced singing voice synthesis system using shallow diffusion conditioned on musical score (lyrics, pitch, MIDI). 44.1kHz output with variance models for pitch, energy, and breathiness control. v2.5.1 released Jan 2026.
...
- **[Ming-omni-tts (inclusionAI)](https://github.com/inclusionAI/Ming-omni-tts)** – Unified autoregressive model generating speech, environmental sounds, and music in a single channel. 0.5B and 16.8B-A3B variants. Fine-grained vocal control (emotion, pitch, dialect), zero-shot voice design from text description. MIT license, released March 2026. [Docs](https://huggingface.co/inclusionAI/Ming-omni-tts-0.5B) | SDK: Python (pip install -r requirements.txt)
...
- **[SoulX-Singer](https://github.com/Soul-AILab/SoulX-Singer)** – Zero-shot singing voice synthesis supporting Mandarin, English, and Cantonese. Trained on 42k hours of vocal data. Supports melody-conditioned (F0) and score-conditioned (MIDI) control. Companion SVC model released March 2026. [Docs](https://soul-ailab.github.io/soulx-singer/)
- **[Step-Audio-EditX](https://github.com/stepfun-ai/Step-Audio-EditX)** – 3B-param RL-trained model for expressive audio editing. Modifies emotion, speaking style, and paralinguistic cues (laugh, exhale, etc.) on existing audio clips. Also supports zero-shot TTS. Apache-2.0. [Docs](https://stepaudiollm.github.io/step-audio-editx/)
...
- **[ThinkSound (FunAudioLLM)](https://github.com/FunAudioLLM/ThinkSound)** – Chain-of-Thought guided any-modality-to-audio framework (video, text, audio input). Supports video-to-audio SOTA, object-centric sound editing, and targeted audio refinement. NeurIPS 2025. pip install thinksound. [Docs](https://huggingface.co/FunAudioLLM/ThinkSound) | SDK: Python (pip install thinksound)
...
- **[RealtimeTTS](https://github.com/KoljaB/RealtimeTTS)** – Python library for streaming TTS with minimal latency. Wraps OpenAI, ElevenLabs, Azure, Kokoro, Piper, Coqui, and 15+ other engines. Automatic fallback across engines. v0.6.0 released March 2026. SDK: Python (pip install realtimetts[all])
...
- **[Kokoro-FastAPI](https://github.com/remsky/Kokoro-FastAPI)** – Dockerized FastAPI wrapper for Kokoro-82M. OpenAI-compatible /v1/audio/speech endpoint with CPU ONNX and NVIDIA GPU PyTorch support. Drop-in replacement for OpenAI TTS. 4700+ stars.
- **[LiveKit Agent Examples](https://github.com/livekit-examples)** – Official examples for building voice/video AI agents.
- **[Voicebox](https://github.com/jamiepine/voicebox)** – Local-first open-source voice synthesis studio with 7 TTS engines, voice cloning, timeline editor, and REST API. Runs on CUDA, Metal, ROCm, and DirectML. MIT license. [Docs](https://voicebox.sh)
